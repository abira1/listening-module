...
(function($, qti) {
        ...
        var applyContentScrolling = function(contentElm, enable) {
            ...
            reviewContainersHeight = function(height, scrollContent, scrollCtnrs) {
                ...
                scrollCtnrs.parents().each(function() {
                    ...
                    $this.children(':visible').not(refCtnr).not('[class*="iScroll"]').each(function() {
                        var $this = $(this).addClass('blockheightfix'),
                            offsetHt = $this.outerHeight(true);
                        height -= offsetHt;
                        $(this).removeClass('blockheightfix');
                    });
            ...
            resizeMain = qti.resizeMain = function() {
                var windowHeight = getWindowHeight(),
                    ...
                    navHt = qti.navHeight = $('#navigation-bar').outerHeight(true),
                    mainHeight = qti.mainHeight = windowHeight - (bannerHt + navHt + 24),
                    // UA 25/07/2018: The 24 above is the total height offset, allowing for the top and bottom paddings,
                    // now applied to the itemBody's #mainContent container in item.css (currently 12px + 12px)
        ...
        qti._setupScrollContentContainers = function() {
            var getScrollBarWidth = function() {
                ...
                optimalTwoColRendering = function($scrollCtnr, $gapMatch) {
                    var $gapsStimulus = $gapMatch.children('[connect\\:class~="gapsStimulus"]'),
                        $gapChoicesCtnr = $gapMatch.children('[connect\\:class~="gapChoicesContainer"]'),
                        scrollCtnrHt = $scrollCtnr.outerHeight() - $gapsStimulus[0].offsetTop;
                    ...
                    $gapChoicesCtnr.css({
                        'height': scrollCtnrHt + 'px',
                        'overflowY': 'scroll'
                    }).append($gapMatch.nextAll('[connect\\:author-class~="container"]'));
                },
                loadFunc,
                loadItemFunc = function(itemBody) {
                    ...
                    if (itemContent.attr('connect:class') === 'columns') {
                        ...
                        if (gapMatches.length === 1 && itemContent.find('>*').length === 1) {
                            gapMatches.find('>*[connect\\:author-class~="left"],>*[connect\\:author-class~="right"]').each(function(index, section) {
                            ...
                            if (gapMatches.length === 1 && gapMatches.eq(0).nextAll().length > 0 &&
                                ...
                                updateGapStimulusHeight = function() {
                                    var $gapMatch = gapMatches.eq(0),
                                        $gapsStimulus = $gapMatch.children('[connect\\:class~="gapsStimulus"]'),
                                        $gapChoicesCtnr = $gapMatch.children('[connect\\:class~="gapChoicesContainer"]'),
                                        gapMatchCtnrsHt = scrollCtnr.outerHeight() - $gapsStimulus[0].offsetTop;

                                    $gapsStimulus.css('height', gapMatchCtnrsHt + 'px');
                                    $gapChoicesCtnr.css('height', gapMatchCtnrsHt + 'px');
                                };
    ...
    (function() {
                ...
                if (navstr) {
                    ...
                    if (rspObj) {
                        if (baseType === 'string') {
                            ...
                            qnElm = qti.activeItem && qti.activeItem.find('span.questionNumber[data-nav="' + navstr + '"]');
                            return (!qnElm || qnElm.length === 0 || qnElm.attr('data-interaction-identifier') === '') ? false : true;
        ...
        var setupItems = qti._setupItems = function() {
            ...
            function findInteractionAnchor(elm) {
                var interaction, col, $intrxnElmt, $intrxnElmts, val;
                if (elm.is('td')) {
                    interaction = elm.parents('[connect\\:responseIdentifier]');
                    if (interaction.is('[connect\\:author-class~="transposeView"],[connect\\:author-class*="matchtype-table-transpose"]')) {
                        if (!elm.is('.row-label')) {
                            col = elm.parent().find('td:not(.row-label)').index(elm);
                            $intrxnElmt = interaction.find('thead th:not(.row-label,.columns-label):eq(' + col + ')');
                        }
                    } else {
                        $intrxnElmt = elm.parent().children(':first');
                    }
                    if ($intrxnElmt && $intrxnElmt.is('[nav],[data-nav]')) {
                        return $intrxnElmt;
                    } else {
                        return null;
                    }
                    ...
                    $intrxnElmts = $();
                    interaction = elm.parents('[connect\\:responseIdentifier]');
                    if (interaction.is('[connect\\:class~="choiceInteraction"]')) {
                        if (interaction.is('.ui-accordion-content')) {
                            interaction = interaction.prev('h3[connect\\:responseIdentifier]');
                        }
                        $intrxnElmts = $intrxnElmts.add(interaction);
                    }
                    val = elm.val();
                    $intrxnElmt = interaction.find('span[data-interaction-identifier="' + val + '"]');
                    if ($intrxnElmt.length === 0) {
                        $intrxnElmt = interaction.find('span[data-interaction-identifier=""]').first();
                    }
                    if ($intrxnElmt.length === 0) {
                        $intrxnElmt = interaction.find('span.questionNumber');
                    }
                    $intrxnElmts = $intrxnElmts.add($intrxnElmt);
                    return $intrxnElmts;
                } else if (elm.is('[nav]')) {
                    return elm;
                } else {
            ...
            function highlightSelection(ancElement) {
                ...
                currentFocus.removeClass('inFocus');
                if (ancElement.is('td,th')) {
                    if (ancElement.parents('[connect\\:responseIdentifier]')
                        .is('[connect\\:author-class~="transposeView"],[connect\\:author-class*="matchtype-table-transpose"]')) {
                        var table = ancElement.parents('table'),
                            col = ancElement.parent().find('td:not(.row-label),th:not(.row-label,.columns-label)').index(ancElement);
                        table.find('tr').each(function() {
                            $(this).find('td:not(.row-label):eq(' + col + ')').addClass('inFocus');
                        });
                    } else {
                        ancElement.parent().find('td').addClass('inFocus');
                    }
                } else if (ancElement.is('.ui-accordion-header, [connect\\:class~="choiceInteraction"], ' +
                        '[connect\\:author-class~="presentation-horizontalPromptOptions"] > span.questionNumber')) {
            ...
            var questionFocusHandler = qti.questionFocusHandler = function(event) {
                ...
                if ($(this).closest('.inFocus').length) {
                    return;
                }
                var intrcn = $(this),
                    isProxyInteraction = intrcn.is('.interaction'),
                    isInlineInteraction = intrcn.is('*[connect\\:class~="inlineInteraction"]') || intrcn.parent().is('*[connect\\:class~="inlineInteraction"]'),
                    interactionsSlctor = '*[connect\\:class~="interaction"], *[connect\\:class~="interaction"] td, *[nav], *[data-nav], .interaction, .maths-entry',
                    ...
                    navref, targetInteractions, dataNav, index, interactions, subInts, ctnrSlctr, activeHighlight, focusTarget, leftHolder;
                if (isInlineInteraction) {
                    navref = intrcn.attr('connect:responseIdentifier');
                    if (!navref) {
                ...
                if (navref && itemref) {
                    ...
                    } else if (!isInlineInteraction && !isProxyInteraction && intrcn.parents('[connect\\:responseIdentifier]').length > 0) {
                        targetInteractions = findInteractionAnchor(intrcn);
                        if (targetInteractions === undefined || targetInteractions === null) {
                            return;
                        }
                        interactionsSlctor = '*[connect\\:responseIdentifier="' + navref + '"] *[nav], *[connect\\:responseIdentifier="' + navref + '"] *[data-nav]';
                        index = qti.activeItem.find(interactionsSlctor).index(targetInteractions.filter('*[nav],*[data-nav]'));
                    } else {
                        interactions = qti.activeItem.find(interactionsSlctor);
                        targetInteractions = interactions.filter('[connect\\:responseIdentifier="' + navref + '"],:has(>[connect\\:responseIdentifier="' + navref + '"])');
                        index = 0;
                    }
                    subInts = targetInteractions.not(function() {
                        var $this = $(this),
                            $intrxn = $this.is('.ui-accordion-header') ? $this.next() : $this,
                            maxChoices = parseInt($intrxn.attr('connect:maxChoices'), 10);
                        return (maxChoices > 1 || maxChoices === 0);
                    }).find('[nav],[data-nav]');
                    if (subInts.length > 1) {
                        return;
                    }
                    if (!targetInteractions.hasClass('inFocus')) {
                        if (targetInteractions.is('*[connect\\:class~="gap"]') &&
                            targetInteractions.closest('*[connect\\:class~="gapMatchInteraction"]').is('*[connect\\:author-class~="presentation-clippedRColHeight"]')) {
                            ctnrSlctr = '[connect\\:author-class~="left"]';
                        } else {
                            ctnrSlctr = '[connect\\:author-class~="right"],[connect\\:class~="scroll-container"]';
                        }
            ...
            function activeInteractionFocus(rspId) {
                ...
                if (targetInteraction.is('[class*="ui-droppable"]') || !targetInteraction.is('[class*="ui-"]')) {
                    if (targetInteraction.find(':input').length) {
                        targetInteraction.find(':input:first').focus();
                    } else {
                        targetInteraction.focus();
                    }
                } else {
                    targetInteraction.focus().click();
                }
            }
    ...
    (function() {
            ...
            $('[connect\\:class~="questionChoice"]').each(function() {
                ...
                content.accordion({
                    ...
                    activate: function(event, ui) {
                        ...
                        var container = ui.newHeader.closest('[connect\\:author-class~="right"],[connect\\:class~="scroll-container"]'),
                            docTop = Math.floor(container.scrollTop()),
        ...
        var doPartInputSetup = function() {
            var rspIds = [],
                replInputs = $('[connect\\:class~="textEntryPart"] input')
                .each(function() {
                    var rspId = $(this).attr('connect:responseIdentifier');
                    if (rspIds.indexOf(rspId) === -1) {
                        rspIds.push(rspId);
            ...
            $.each(rspIds, function(i, rspId) {
                var inputSet = replInputs.filter('[connect\\:responseIdentifier="' + rspId + '"]'),
                    ...
                    rspArray = [],
                    placeholderText;
                inputSet.attr('size', tei.attr('size'));

                if (rspVar.getValue()) {
                    if (inputSet.length > 1) {
                        rspArray = rspVar.getValue().split(",");
                    } else {
                        rspArray.push(rspVar.getValue());
                    }

                    $.each(inputSet, function(j, obj) {
                        var $this = $(this),
                            valueText = (rspArray[j]) ? rspArray[j].split(":")[1] : '';
                        if (valueText) {
                            $(this).val(valueText);
                            placeholderText = $this.prev("span.questionNumber")[0].innerHTML;
                            $($this.prev("span.questionNumber")[0]).hide();
                            tei[0].updateValue();
                        }
                    });
                }

                inputSet.bind('keyup', function() {
                    var $this = $(this);
                    placeholderText = $this.prev("span.questionNumber")[0].innerHTML;
                    $($this.prev("span.questionNumber")[0]).hide();
                    var textPartResponses = [];
                    $.each(inputSet, function(j, obj) {
                        textPartResponses.push('Gap' + (j + 1) + ':' + inputSet[j].value);
                    });
                    var teiValue = textPartResponses.join(',');
                    tei.val(teiValue);
                    tei[0].updateValue();
                }).bind('focus', function(e, obj) {
                    var $this = $(this);
                    placeholderText = $this.prev("span.questionNumber")[0].innerHTML;
                    $($this.prev("span.questionNumber")[0]).hide();
                }).bind('blur', function(e, obj) {
                    var $this = $(this);
                    $this.parent().addClass("bardo");
                    if ($this.value() === '' && placeholderText) {
                        $($this.prev("span.questionNumber")[0]).show();
                    }
                });
        ...
        qti.setupQuestionChoicePresentations = function() {
            ...
            if ($('[connect\\:class~="textEntryPart"]').length > 0) {
                qti.subscribeToEvent("testStart", doPartInputSetup, "Setup text-entry inputs for questions with part responses", "setupPartInputs");
            }
        };
    ...
    (function() {
        qti._setupAudio = function() {
            if ($('#audio-content, #main-audio-content').length > 0) {
                var $audioControl = $('#audio-control'),
                    $audioContent = $('#audio-content, #main-audio-content').eq(0),
                ...
                ap.$controls = $audioControl;
                ap.$mainAudio = $audioContent;
        ...
        var loadItemFunc = function(itemBody) {
                ...
                var setupTextInteraction = function(textInteraction) {
                        ...
                        textInteraction.keypress(function(event) {
                            ...
                            // Update textbox but ignore for backspace and delete button press
                            if (expandable && event.keyCode !== 8 && event.keyCode !== 46) {
                                valueText = textInteraction.value();
                                resizeInteraction();
                            }
                        ...
                        if (expandable) {
                            textInteraction.keyup(function(event) {
                                if (keywordEntry && event.charCode === 32) {
                                    return false;
                                }
                                // Manage backspace amd delete button click
            ...
            loadItemFunc = function(itemBody) {
                ...
                var textInteractions,
                    setupExtTextInteraction = qti.setupExtTextInteraction = function(textInteraction, rspId, itemId) {
                        ...
                        var regex = /<br\s*[\/]?>/gi;
                        valueText = valueText.replace(regex, "\n");
    ...
    (function() {
        ...
        var gapChoicesCtnr = '>div[connect\\:class~="gapChoicesContainer"]',
            dragContainerSlctr = gapChoicesCtnr + '>*[connect\\:class~="gapChoices"]',
            dropContainerSlctr = '>div[connect\\:class~="gapsStimulus"]',
            loadItemFunc = function(itemBody) {
                ...
                interactions.each(function() {
                    var interaction = $(this),
                        ...
                        hasInlineGapChoices = interaction.is('[connect\\:author-class~="presentation-ortn-vertical"],[connect\\:author-class~="presentation-gapchoices-inline"]'),
                        $gapChoicesContainer = interaction.find(gapChoicesCtnr),
                        gapsize = 75,
                        width = 16,

                        //Locate drag and drop targets
                        dragItems = interaction.find(dragContainerSlctr + '>*[connect\\:class~="' + dragTargetClass + '"]'),
                        dropItems = interaction.find(dropContainerSlctr + ' *[connect\\:class~="' + dropTargetClass + '"]'),
                        ...
                        dragContainer = interaction.find(dragContainerSlctr);
                    ...
                    (function() {
                        ...
                        // UA 24/07/2018: variables used in laying out drag items using position absolute;
                        var dragItemTopOffset, gapChoicesPaddingLeft, gapChoicesHeightOffset;

                        function layoutDragItems() {
                            if (!hasInlineGapChoices) {
                                dragItemTopOffset = 0;
                                if (!gapChoicesPaddingLeft) {
                                    gapChoicesPaddingLeft = parseInt(dragContainer.css('padding-left'), 10);
                                }
                                dragContainer.children(':visible').each(function(i) {
                                    var dragItem = $(this),
                                        itemHeight = dragItem.height();
                                    if (i === 0) {
                                        if (!gapChoicesHeightOffset) {
                                            gapChoicesHeightOffset = dragItem.outerHeight(true) - itemHeight;
                                        }
                                        if (gapChoicesHeightOffset) {
                                            dragItemTopOffset += (gapChoicesHeightOffset / 2);
                                        }
                                    }
                                    dragItem.css({
                                        position: 'absolute',
                                        top: dragItemTopOffset,
                                        left: gapChoicesPaddingLeft,
                                        marginTop: 0,
                                        marginBottom: 0
                                    });
                                    dragItemTopOffset += (itemHeight + gapChoicesHeightOffset);
                                });
                                if (gapChoicesHeightOffset) {
                                    dragItemTopOffset -= (gapChoicesHeightOffset / 2);
                                }
                                dragContainer.css('height', dragItemTopOffset + "px");
                            }
                        }
                        ...
                        dropItems.each(function(index, dropItem) {
                            ...
                            if (value.size() !== 0) {
                                ...
                                if (dragId) {
                                    dragItem = interaction.find(dragContainerSlctr + '>*[connect\\:identifier="' + dragId + '"]');
                        ...
                        function $makeDraggable(elm, options) {
                            ...
                            var settings = {
                                ...
                                appendTo: $gapChoicesContainer[0],
                                zIndex: 100,
                                containment: interaction[0]
                            };
                            if (options) {
            ...
            manageInteractionHeight = function(item) {
                if (item) {
                    ...
                    $gapMatches.each(function() {
                        ...
                        if (stimHeight > scrollContainerHeight) {
                            $stim.css('height', scrollContainerHeight + 'px');
                        } else {
                ...
                var savedRspVals = {},
                    setGMI_State = function(rspVar) {
                        var rspId = rspVar.getIdentifier().split('.')[1],
                            ...
                            rspVal, nvals, savedRspVal, gapId;
                        ...
                        if (gmi.is('[connect\\:class~="gapMatchInteraction"]')) {
                            ...
                            savedRspVal = savedRspVals[rspId];
                            if (!savedRspVal || nvals > savedRspVal.values().length) {
                                if (rspVal) {
                                    gapId = rspVal.keys()[nvals - 1];
                                    gmi.find(dropContainerSlctr + ' *[connect\\:identifier~="' + gapId + '"]').addClass('attempted');
                                }
                            } else if (savedRspVal) {
                                gapId = (rspVal) ? diffArray(savedRspVal.keys(), rspVal.keys())[0] : savedRspVal.keys();
                                gmi.find(dropContainerSlctr + ' *[connect\\:identifier~="' + gapId + '"]').removeClass('attempted');
                            }

                            if (rspVal) {
                                savedRspVals[rspId] = $.extend({}, rspVal); // clone the rspVal object
                            } else {
                                savedRspVals[rspId] = undefined;
                            }
                        }
                    };
        ...
        var loadItemFunc = function(itemBody) {
            ...
            */
            ...
            choiceInteractions.each(function(index, choiceInteraction) {
                ...
                var valueParsers = $.createHash('directedPair', qti.parseDirectedPair),
                    baseType = responseVariable.getBaseType();
                if (baseType !== 'identifier' && baseType !== 'directedPair') {
                    throw new Error('Invalid base type');
                } else if (baseType === 'directedPair' && !isMultiNav && !valueParsers.has(baseType)) {
                    throw new Error('Invalid base type');
                }
                ...
                inputs.click(function() {
                    if (isMultiNav) {
                        var vals = inputs.value(),
                            value;
                        if (vals.length) {
                            value = $.createHash();
                            vals.forEach(function(val, i) {
                                value.set("Gap" + (i + 1), val);
                            });
                            responseVariable.setValue(value);
                        } else {
                            responseVariable.setValue(undefined);
    ...
    (function() {
        ...
        var interactionLineHashes = $.createHash(),
            ...
            transposeTable = function($table) {
                var $thead_rows = $table.find('thead tr'),
                    i = 0,
                    $questions_title, $choices_title, $title,
                    $tb, $tr, $cell, $newtb, $newtr, cols, rows, col;

                if ($thead_rows.length === 2) {
                    $questions_title = $thead_rows.eq(0).children('th').eq(0).detach();
                    $choices_title = $thead_rows.eq(0).children('th').eq(0).removeAttr('colspan').detach();
                    /* UA 30/05/2018: ...('th').eq(0) is now what was previously ('th').eq(1), following earlier detachment! */
                    $choices_title.prependTo($thead_rows.eq(1));
                    $thead_rows.eq(0).remove();
                    //} else {
                    //$questions_title = $thead_rows.eq(0).children('th').eq(0).clone()
                }
                $table.find('thead tr').detach().prependTo($table.find('tbody'));

                $tb = $table.find('tbody').eq(0);
                $tr = $tb.find('tr');
                cols = $tr.length;
                rows = $tr.eq(0).find('td,th').length;
                $newtb = $('<tbody></tbody>');
                ...
                while (i < rows) {
                    ...
                    $newtr = $('<tr></tr>');
                    while (col < cols) {
                        $cell = $tr.eq(col++).find('td,th').eq(0);
                        $newtr.append($cell);
                    }
                    $newtb.append($newtr);
                    ++i;
                ...
                $newtb.appendTo($table);

                $table.find('tbody tr:eq(0)')
                    .detach()
                    .appendTo($table.find('thead'))
                    .children()
                    .each(function(i) {
                        var $this = $(this),
                            $trgt = $('<th scope="col">' + $this.html() + '</th>');
                        $this.copyAttributes($trgt);
                        if (i === 0) {
                            $trgt.addClass('row-label');
                        }
                        $this.replaceWith($trgt);
                    });

                $table.find('tbody tr th:first-child')
                    .each(function() {
                        var $this = $(this),
                            $trgt = $('<td scope="row">' + $this.html() + '</td>');
                        $this.copyAttributes($trgt);
                        $trgt.addClass('row-label');
                        $this.replaceWith($trgt);
                    });

                if ($questions_title && $questions_title.length) {
                    $thead_rows = $table.find('thead tr');
                    $title = $thead_rows.eq(0).children('th').eq(0);
                    $questions_title.copyAttributes($title);
                    $questions_title.removeAttr('rowspan').removeAttr('style');
                    $questions_title.attr('colspan', cols - 1).addClass('columns-label');
                    $newtr = $('<tr></tr>').append($title).append($questions_title);
                    $table.find('thead').prepend($newtr);
                }
                $table.show();
            },
        ...
        var loadItemFunc = function(itemBody) {
            ...
            tableMatchInteractions.each(function(index, tableMatchInteraction) {
                ...
                if (interaction.is('[connect\\:author-class~="separateStimulus"]')) {
                    ...
                    interaction.find('tbody td:has(span.questionNumber)').each(function() {
                        var $this = $(this),
                ...
                interaction.find('thead th, tbody td').each(function() {
                    var $this = $(this),
                        $thead_rows = interaction.find('thead tr'),
                        $tbody_rows = interaction.find('tbody tr'),
                        $firstRowOfCells = $tbody_rows.first().find('td:not(.row-label)');

                    if ($this.is('th[connect\\:identifier]') && $this.is('[data-nav]')) {
                        $this.attr('tabindex', "0");
                        $this.on('focus click', function(event) {
                            var i = $thead_rows.find('th:not(.row-label,.columns-label)').index($this[0]);
                            $firstRowOfCells.eq(i).triggerHandler('focus');
                            $tbody_rows.each(function() {
                                var $trgt_td = $(this).find('td:not(.row-label)').eq(i);
                                if ($trgt_td.is('.checked')) {
                                    $trgt_td.focus();
                                    return false;
                                }
                            });
                        });
                    } else if ($this.is('td[connect\\:identifier]') && $this.is('[data-nav]')) {
                        $this.attr('tabindex', "0");
                        $this.on('focus click', function(event) {
                            var $checked = $this.parent().find('td.checked');
                            if ($checked.length) {
                                $checked.focus();
                            }
                        });
                    } else if ($this.is('td') && !$this.is('[connect\\:identifier]')) {
                        $this.attr('tabindex', "0");
                    }
