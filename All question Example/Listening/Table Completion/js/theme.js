...
var theme = (function($) {
    ...
    (function() {
        var flashChanges = 20,
            ...
            timerMarkup = function() {
                ...
                qti.timerMarkup.displayValue = $('span[connect\\:function~="timer"] > span.time-value');
                qti.timerMarkup.displayText = $('span[connect\\:function~="timer"] > span.time-text');
    ...
    (function() {
        ...
        function preparePreTestAudio() {
            var btnText,
                $sampleAudio = $('#sample-audio-content');
            if (qti.audioPlayer && $sampleAudio.length > 0) {
                qti.audioPlayer.jPlayer("setMedia", {
                    oga: $sampleAudio.val()
                });
                if (qti.audioPlayer.$controls && qti.audioPlayer.$controls.length) {
                    qti.audioPlayer.$controls.show();
                }
                $('input:button.sound').on('click', function() {
                    if ($(this).is('.playing')) {
        ...
        function preloadMainAudio() {
            if (qti.audioPlayer) {
                var $mainAudio, audioResource, playList,
                    primeAudio = function() {
                        ...
                        //qti.audioPlayer.jPlayer("play");
                        //qti.audioPlayer.jPlayer("stop");
                        qti.audioPlayer.resource = audioResource;
                    };
                if (qti.audioPlayer.$controls && qti.audioPlayer.$controls.length) {
                    qti.audioPlayer.$controls.show();
                }
                if (qti.audioPlayer.$mainAudio && qti.audioPlayer.$mainAudio.length) {
                    if (qti.audioPlayer.is('#audio-content')) {
                        $mainAudio = $('#main-audio-content');
                        qti.audioPlayer = $mainAudio;
                        qti.audioPlayer.$mainAudio = $mainAudio;
                    } else {
                        $mainAudio = qti.audioPlayer.$mainAudio;
                    }

                    audioResource = $mainAudio.attr('data-resource') || $mainAudio.val();
                    if (audioResource && $mainAudio.is('.playlist')) {
                        playList = qti.audioPlayer.playList = new jPlayerPlaylist({
                            jPlayer: "#main-audio-content"
                        }, [], {
                            playlistOptions: {
                        ...
                        $.getJSON(audioResource, function(data) {
                            if (data instanceof Array) {
                                var audioPlayList = data.map(function(item) {
                                    return {
                                        oga: "test-content/" + item.filename
                                    };
                                });
                                playList.setPlaylist(audioPlayList);
                                playList.data = data;
                                primeAudio();
                            }
                        });
        ...
        function prepareTestScreen() {
            ...
            qti._onResize();
            if (qti.audioPlayer) {
                if (qti.audioPlayer.resource) {
                    if (qti.audioPlayer.playList) {
                        var startTime = qti.audioPlayer.mainAudioStart || 0,
                            audioData = qti.audioPlayer.playList.data,
                            itemDuration, elapsedTime = 0,
                            i;
                        for (i = 0; i < audioData.length; i++) {
                            itemDuration = Number(audioData[i].duration);
                            elapsedTime += itemDuration;
                            if (elapsedTime >= (qti.audioPlayer.mainAudioStart || 0)) {
                                qti.audioPlayer.playList.select(i);
                                break;
                            } else {
                                startTime -= itemDuration;
                            }
                        }
                        qti.audioPlayer.jPlayer("play", startTime);
                    } else {
                        qti.audioPlayer.jPlayer("play", qti.audioPlayer.mainAudioStart);
                    }
                } else {
                    var audioResource = qti.activeItem.find('audio:first').attr("src");
                    if (audioResource) {
                        qti.audioPlayer.jPlayer("setMedia", {
                            oga: audioResource
                        });
