...
var qti = (function($) {
    ...
    (function() {
        ...
        function loadJS(url, jsQueue, loadedJS) {
            var promises = [],
            ...
            url.split(',').forEach(function(url) {
                ...
                if (s > 0) {
                    loadedJS[jsQueue[s - 1]].done(addScript);
                } else {
                    addScript();
                }
        ...
        qti.loadScripts = function(scripts) {
            var dfrd = $.Deferred(),
                jsQueue = [],
                loadedJS = {};

            scripts.forEach(function(jsUrl) {
                $.when.apply(null, loadJS(jsUrl, jsQueue, loadedJS)).done(function() {
        ...
        var processSVGInteractions = qti.processSVGInteractions = function($svgDoc, $svgObj) {
            var svgCtnr, svgTexts, svgObj,
                svgW = $svgDoc.attr('width'),
                svgH = $svgDoc.attr('height');
            ...
            svgCtnr.css('width', svgW).css('height', svgH);
            svgTexts = $svgDoc.find('text[class="interaction"]');
            if (svgTexts.length === 0) {
                ...
                svgTexts.each(function() {
                    ...
                    var svgPlaceholderSpan = svgTextElm.find('tspan[class="interaction-placeholder"]'),
                        svgPlaceholderWidth = 0,
                        svgPlaceholderX = 0,
                        svgPlaceholderY = 0,
                        svgPlaceholderParentX = 0,
                        svgPlaceholderParentY = 0,
                        svgPlaceholderText = "";
                    ...
                    if (svgPlaceholderSpan[0]) {
                        ...
                        svgPlaceholderWidth = svgPlaceholderSpan[0].getBoundingClientRect().width;
                    ...
                    var svgTxtW = svgTextElm[0].getBoundingClientRect().width,
                        svgTxtH = svgTextElm[0].getBoundingClientRect().height,
                        svgTxtPosL = parseInt(svgTextElm.attr('x'), 10),
                        svgTxtPosT = parseInt(svgTextElm.attr('y'), 10),
                        rspid = svgTextElm.attr('id'),
                        htmlTeiW, htmlTeiH, htmlTxtX, htmlTxtY, tei, teiInput,
                        txt = svgTextElm.text(),
                        indexLastDash, respIdLeader, respIdTrailer, newSpanX, newSpanY;

                    if (svgPlaceholderWidth === 0) {
                        svgTextElm.text('');
                    } else {
                        svgPlaceholderSpan.css("visibility", "hidden");
                    }

                    tei = svgCtnr.find('span[connect\\:class~="inlineInteraction"]:has([connect\\:responseIdentifier="' + rspid + '"]), [connect\\:identifier="' + rspid + '"]');
                    htmlTeiW = tei.outerWidth();
                    htmlTeiH = tei.outerHeight();
                    htmlTxtX = Math.round((svgTxtPosL - (htmlTeiW / 2)) / svgW * 99 * 100) / 100; // - svgCvnsPos.left + (svgTxtW / 2)
                    htmlTxtY = Math.round((svgTxtPosT - (svgTxtH / 2)) / svgH * 99 * 100) / 100; // - svgCvnsPos.top - (htmlTeiH / 2)
                        ...
                        if (svgPlaceholderWidth === 0) {
                            ...
                            if (svgPlaceholderText.indexOf("text entry") !== -1) {
                                ...
                                tei.css({
                                    ...
                                    top: svgPlaceholderY + 'px',
                                    left: svgPlaceholderX + 'px',
                                    minWidth: svgPlaceholderWidth + 'px',
                                    padding: 0,
                                    margin: 0,
                                    fontSize: svgFontSize + "px"
                                });
        ...
        $.fn.setSVGStimulusInteractions = function() {
            ...
            if (svgDoc === null || $svgDoc.find('svg,svg\\:svg').length === 0) {
                ...
                processSVGInteractions($svgDoc, $svgObj);
            }
        };
        ...
        var generateSVG = function(container) {

            var siblingCount = container.siblings("svg").length;

            var getImages = function() {
                var accessImages = container.find("image");
                if (accessImages.length === 1 && siblingCount === 0) {
                    var newImage = new Image();
                    var rootImage = $(accessImages[0]);
            ...
            var isSVGInteraction = container.attr("connect:author-class").indexOf("accessible-svg") !== -1;

            if (siblingCount === 0) {
                var embedPNG = container.find("image");
                var interactionContent = "";

                if (isSVGInteraction) {
                    container.attr("connect:author-class", "accessible-svg");
                    container.find('tspan[class="interaction-placeholder"]').css({
                        "visibility": "hidden"
                    });
                } else {
                    container.removeAttr("connect:author-class");
                }

                if (embedPNG.length === 0 || isSVGInteraction) {
                    var XMLS = new window.XMLSerializer();
                    var imageSource = XMLS.serializeToString(container[0]);
                    var sourceHeight = container[0].getAttribute("height");
                    var sourceWidth = container[0].getAttribute("width");
                    var convertedSource = window.btoa(imageSource);
                    var pngSource = 'data:image/svg+xml;base64,' + convertedSource;
                    var canvas = document.createElement("canvas");
                    var ctx = canvas.getContext("2d");
                    var imageObj = new Image();
                    imageObj.onload = function() {
                        ...
                        var mImage = document.createElementNS("http://www.w3.org/2000/svg", "image");
                        mImage.setAttribute("id", "svg_1");
                        mImage.setAttribute("x", "0");
                        mImage.setAttribute("y", "0");
                        mImage.setAttribute("height", sourceHeight);
                        mImage.setAttribute("width", sourceWidth);
                        ...
                        var oldGroup = container[0].getElementsByTagNameNS("http://www.w3.org/2000/svg", "g")[0];
                        container[0].replaceChild(mGroup, oldGroup);
                        getImages();
                    };
                ...
                if (isSVGInteraction) {
                    var svgImages = container.siblings("svg");
                    svgImages.each(function() {
                        var svgImage = $(this);
                        var imageClass = svgImage.attr("connect:author-class");
                        var textMaster = container[0].getElementsByTagNameNS("http://www.w3.org/2000/svg", "text");
                        var newGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
                        for (var i = 0; i < textMaster.length; i++) {
                            var textNode = textMaster[i].cloneNode(true);
            ...
            if (docObj) {
                ...
                if (qti.accessibilityImages.indexOf(itemIdentifier) === -1) {
                    ...
                    images.each(function() {
                        var image = $(this);
                        var imageElement = this.nodeName;
    ...
    (function() {
        ...
        var applyContentScrolling = function(contentElm, enable) {
            ...
            checkScrollContentHeight = function(crntItemBody) {
                var maxScrollHeight, preItemsOffset = 0,
                    itemHeight = crntItemBody.height(),
                    scrollContent = crntItemBody.find('*[connect\\:class~="scroll-content"]');
                if (scrollContent.length > 0) {
                    /* crntItemBody.find('>div').each(function() {
                    	var childDiv = $(this)
                    	if (!/scroll-content/.test(childDiv.attr('connect:class'))) {
                    		preItemsOffset += childDiv.outerHeight();
                    	} else {
                    		return false;
                    	}
                    });
                    maxScrollHeight = qti.mainHeight - (preItemsOffset + 44); */
                    preItemsOffset = Math.round(scrollContent.position().top);
                    maxScrollHeight = qti.mainHeight - preItemsOffset;
                    updateScrollView(scrollContent, maxScrollHeight, itemHeight, crntItemBody.is('[connect\\:identifier]'));
            ...
            resizeMain = qti.resizeMain = function() {
                var windowHeight = getWindowHeight(),
                    ...
                    currentItem = mainContent.find('*[connect\\:class~="activeItem"]');
                ...
                if (currentItem.length > 0) {
                    ...
                    currentItem = mainContent.find('*[connect\\:class~="itemBody"]:first');
                }
        ...
        qti._setupScrollContentContainers = function() {
            var getScrollBarWidth = function() {
                ...
                optimalTwoColRendering = function($scrollCtnr, $gapMatch) {
                    ...
                    $gapMatch.next('[connect\\:author-class~="container"]').css({
        ...
        (function() {
            ...
            (function() {
                function getControls(xml) {
                    content = xml.find('[data-instructionsControls]');
                    content = content[0];
            ...
            var accessDialogue = (function() {
                ...
                var openDialogue = function(dialogueName) {
                    $.dialogue({
                        title: dialogueName, //'Universal access',
                        hasTopClose: true,
            ...
            var helpDialogue = (function() {
                ...
                helpDialogue.showDialogue = function(dialogueName, draggable) {
                    openDialogue(dialogueName);
                    if (qti.videoPlayer && $(content).find('div.video-js-box').length > 0) {
            ...
            var lockDialogue = (function() {
                ...
                lockDialogue.showDialogue = function(dialogueName) {
                    openDialogue(dialogueName);
                };
            ...
            (function() {
                function attachLinkEvent() {
                    ...
                    $('[connect\\:class~="lockLink"]').each(function(index, fmLink) {
                        fmLink = $(fmLink);
                        fmLink.click(function(event) {
                            lockDialogue.showDialogue("Screen hidden");
                            event.preventDefault();
                            return false;
                        });
                    });
        ...
        */
        ...
        function serializeDirectedPairs(elem, multiVal) {
            ...
            var a = 0,
                ...
                maxAnswers = qti.getVariable(rspId).getMaxChoices();
        ...
        var createSerializedXMLResults = qti.createSerializedXMLResults = function(id, baseType, cardinality, state, valueObj) {
            ...
            if (idArray[0] === 'candidateInformation') {
                ...
                XMLItemVariable = $.el("", "candidateInformationResponse");
            } else {
                valueElement = "value";
                XMLItemVariable = $.el("", "response", XMLItemVariableAttributes);
            }
            XMLItemVariable = cardinalities[cardinality].func(baseType, valueObj, XMLItemVariable);
        ...
        */
        ...
        var deserializeXML = qti.deserializeXML = function(xml) {
            ...
            function constructDirectedPair(val) {
                return qti.parseDirectedPair(val.text());
            }
            ...
            function constructDirectedPairs(valElm) {
                var valArray = [],
                    vals = valElm.text();
                if (vals) {
                    vals = vals.split(',');
            ...
            function constructPoint(val) {
                return qti.parsePoint(val.text());
            }
            ...
            var baseTypes = {
                ...
                "directedPair": {
                    "deSerializer": constructDirectedPairs
                },
                "point": {
    ...
    (function() {
        ...
        var checkCompositeInteractionStatus =
            qti.checkCompositeInteractionStatus = function(itemId, rspId, aElm) {
                ...
                if (navstr) {
                    ...
                    if (rspObj) {
                        if (typeof rspObj[0] === 'string') {
                            qnElm = qti.activeItem.find('span.questionNumber[data-nav="' + navstr + '"]');
                            return (qnElm.attr('data-interaction-identifier') === '') ? false : true;
                        } else {
                            subIntId = navstr.slice(rspId.length + 1);
        ...
        function checkResponseStatus(item, aElement) {
            ...
            } else if (aElement.attr('nav')) {
                interactions = aElement.attr('nav').split("|");
        ...
        var setupItems = qti._setupItems = function() {
            ...
            function highlightSelection(ancElement) {
                var currentFocus = mainElement.find('.inFocus');
            ...
            var questionFocusHandler = qti.questionFocusHandler = function(event) {
                ...
                var intrcn = $(this),
                    ...
                    targetInteractions, index, subInts, ctnrSlctr;
                if (!navref) {
                ...
                if (navref && itemref) {
                    ...
                    if (intrcn.parents('[connect\\:responseIdentifier]').length > 0) {
                    ...
                    if (!targetInteractions.hasClass('inFocus')) {
                        ...
                        qti.highlightNavItem(navref, itemref, index);

                        // related keywords
                        var k = (targetInteractions.is('[connect\\:responseIdentifier]')) ?
                        ...
                        $('em[connect\\:author-class~="' + k + ' highlight"]').addClass('inFocus');
                        ...
                        // separateStimulus
                        qti.activeItem.find('div.separateStimulus *[data-interaction-identifier]')
                            .eq(index).addClass('inFocus');
                    }
                }
            };
            ...
            function activeInteractionFocus(rspId) {
                ...
                if (!targetInteraction || !targetInteraction.is('[connect\\:class~="interaction"],[connect\\:responseIdentifier],[connect\\:identifier],.interaction')) {
                    if (rspId) {
            ...
            qti.highlightNavItem = function(navref, itemref, index) {
                var item, aElement = $('#navigation-bar a[href$="' + navref + '"]');
            ...
            var displayItem = qti.displayItem = function(itemBody) {
                    var screenId;
                    mainElement.find('>[connect\\:class~="activeItem"]').removeAttrToken('connect:class', 'activeItem').hide();
                    if (itemBody && typeof itemBody !== "object") {
                        screenId = itemBody;
                        qti.activeItem = mainElement.find('>*[connect\\:identifier="' + itemBody + '"]');
                        if (qti.activeItem.length === 0) {
                            qti.activeItem = mainElement.find('>*[connect\\:groupIdentifier~="' + itemBody + '"]');
                    ...
                    $.when.apply($, dfrdSet).done(function() {
                        var itemBody, identifier, groupItem,
                            ...
                            handleStimulus = function() {
                                var $this = $(this),
                                    ...
                                    idfr = $this.attr('connect:author-id'),
                                    contentDiv = $('<div class="async-stimulus"/>'),
                                    loadContent = function(url, dataType) {
                                        return $.get(url, function(content) {
                                            if (dataType === "xml") {
                                                content = document.importNode(content.documentElement, true);
                                                if (author_class[url]) {
                                                    $(content).attrNS('connect:author-class', author_class[url]);
                                                }
                                            }
                                ...
                                if (url) {
                                    ...
                                    $this.find('object[type="image/svg+xml"]').each(function(i) {
                                        var $obj = $(this),
                                            url = $obj.attr('data');
                                        if (!idfr && i === 0) {
                                            idfr = url; // use the URL of the first object as the identifier for the SVG-image-set container
                                        }
                                        urls.push(url);
                                        ...
                                        if ($obj.is('[connect\\:author-class~="accessible-svg"]') ||
                                            ...
                                            author_class[url] = "accessible-svg " + $obj.attr('connect:author-class');
                                        } else {
                                            author_class[url] = $obj.attr('connect:author-class');
                                        }
                                        $obj.remove();
                                    });
                                }
                                ...
                                if (idfr && external_stimuli.indexOf(idfr) === -1) {
                                    urls.forEach(function(url) {
                                        var dataType = (/\.html$/.test(url) || /\.htm$/.test(url)) ? 'html' : 'xml',
                                            promise = loadContent(url, dataType);
                                        extStiPromises.push(promise);
                                    });

                                    external_stimuli.push(idfr);
                                    contentDiv.hide().attr('id', idfr).appendTo(mainElement);
                                }
                            },
                            finalSetup = function() {
                                loader.remove();
                        ...
                        for (identifier in itemsXML) {
                            if (itemsXML.hasOwnProperty(identifier)) {
                                ...
                                mainElement.append(itemBody);
                            }
                        }

                        //qti.fireEvent("itemsPreloaded", mainElement);

                        qti.setupQuestionChoicePresentations();
                        if (item.group) {
                            groupItem = mainElement.find('[connect\\:groupIdentifier~="' + item.group + '"]');
                            if (groupItem.length === 0) {
                                groupItem = mainElement.find('[connect\\:identifier="' + item.group + '"]');
                            }
                            qti.activeItem = groupItem;
                        } else {
                            qti.activeItem = mainElement.find('*[connect\\:class~="itemBody"]:not(.nontest):first');
                        }
                ...
                changeItem = qti.changeItem = function(item, aIndexElement, callback) {
                    ...
                    var targetRef = (aIndexElement ? aIndexElement.href().getFragment() : undefined),
                        ...
                        onItemLoaded = function(itemDocument) {
                            ...
                            mainElement.find('>[connect\\:class~="itemBody"]').remove();
                            mainElement.append(itemBody);
            ...
            var initialiseScreen = function(itemBody) {
                ...
                itemBody.find('[connect\\:author-class="svg-interactions-container"] svg').each(function() {
                    qti.processSVGInteractions($(this));
                });
            ...
            assessmentItemRefElements.each(function(index, assessmentItemRefElement) {
                ...
                aElement.each(function(index, aIndexElement) {
                    ...
                    if (targetRef) {
                        ...
                        if (aIndexElement.attr('nav')) {
                            intrcnIds = aIndexElement.attr('nav').split("|");
            ...
            $('[connect\\:class~="floatingTool"] a[href!=""][href!="#"]').each(function(i, toolLink) {
                ...
                function bringToFront() {
                    if (frontToolWindow) {
                        frontToolWindow.css('zIndex', 1);
                    }
                    toolWindow.css('zIndex', 2);
    ...
    (function() {
        function setupAccordionGroupByItem(group) {
            var tmplRef = qti.itemGroups[group].items[0],
                groupItem = $(qti.mainElement.find('[connect\\:identifier="' + tmplRef + '"]')[0].cloneNode(true));
            groupItem.attrNS('connect:identifier', group);
            //groupItem.attrNS('connect:groupIdentifier', group);
            qti.mainElement.append(groupItem);
            ...
            for (i = 0; i < qti.itemGroups[group].items.length; i++) {
                ...
                srcItem = qti.mainElement.find('[connect\\:identifier="' + itemId + '"]');
        ...
        function setupAccordionGroupByInteraction(group) {
            var tmplRef = qti.itemGroups[group].items[0],
                groupItem = qti.mainElement.find('[connect\\:identifier="' + tmplRef + '"]'),
        ...
        qti.setupSharedStimuli = function() {
            var loadItemFunc = function(itemObj) {
                    var stimulus = itemObj.find('div.external-stimulus, *[connect\\:author-class~="async-stimulus"]');
                    if (stimulus.length > 0) {
                        stimulus.each(function() {
                            var stimulusDiv = $(this),
                                stimulusId = stimulusDiv.attr('data-source') || stimulusDiv.attr('connect:author-id');
                            $('div[id="' + stimulusId + '"]').children().appendTo(stimulusDiv);
                        });
                    }
                },
                unloadItemFunc = function(itemObj) {
                    ...
                    if (stimulus.length > 0) {
                        stimulus.each(function() {
                            var stimulusDiv = $(this),
                                stimulusId = stimulusDiv.attr('data-source') || stimulusDiv.attr('connect:author-id');
                            stimulusDiv.children().appendTo('div[id="' + stimulusId + '"]');
                        });
                    }
                };
    ...
    (function() {
        ...
        function checkSelection(opt) {
            ...
            if (screen.sel.rangeCount === 0 || screen.sel.isCollapsed) {
                ...
                if ($(screen.crntTarget).is('.ylw-hglted')) {
        ...
        function clearAllHighlight() {
            ...
            for (key in ranges) {
                if (ranges.hasOwnProperty(key)) {
                    var trgtRng = ranges[key];
                    if ($(trgtRng.startContainer).parent().closest('[connect\\:class~="activeItem"]').length > 0) {
                        cssApplier.undoToRange(trgtRng);
                        if (screen.notes[key]) {
                            $(screen.notes[key].note).remove();
                            delete screen.notes[key];
        ...
        qti._setupTextHighlighting = function(options) {
            ...
            $('[role~="banner"], [role~="main"], [role~="navigation"]').bind('mousedown', function(e) {
                var target, interactions, respId,
                    fadeMaybe = false,
                    regexp = /#\w+-\w+-\w+-\w+/, // matches a hash character followed by a response identifier
            ...
            $('[role~="main"]').on('mouseup', '.ylw-hglted', function(event) {
            ...
            $('[role~="main"]').on('mouseover', '.ylw-hglted', function(event) {
            ...
            $('[role~="main"]').on('mouseleave', '.ylw-hglted', function(event) {
                var htext = event.target,
                    rkey = getContextualRange(htext);
                if (rkey && screen.notes[rkey]) {
            ...
            $('[role~="main"]').on('mouseout', '.notesIcon', function(event) {
                $(this).hide();
            });
        ...
        var setupFormattedContent = function(itemObj) {
                ...
                var setupTextInteraction = function(textInteraction) {
                        ...
                        function resizeInteraction() {
                            ...
                            if (valueText === '') {
                                ...
                                lPx = (lPx > parentWidth) ? parentWidth : ((lPx > emptyWidthPx) ? lPx : emptyWidthPx);
                            }
            ...
            loadItemFunc = function(itemBody) {
                ...
                var textInteractions,
                    setupExtTextInteraction = qti.setupExtTextInteraction = function(textInteraction, rspId, itemId) {
                        ...
                        textInteraction.on('change', onFocusOrBlur);
                        onFocusOrBlur();
                    };
            ...
            setupExtendedTextRTE = function(itemInteraction, textInteraction) {
                var proxyTextareaElm = textInteraction.clone(),
                    ...
                    settings = {
                        ...
                        plugins: [
                            "advlist contextmenu charmap code fullscreen lists table visualblocks"
                        ],
                        menubar: false,
                        statusbar: false,
                        toolbar: "insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist", //+ (ME ? " mathsentry" : ""),
                        content_css: "js/maths-entry/maths-edit.rte.css",
                        content_js: (ME ? "js/jquery.js,js/maths-entry/maths-edit.rte.js" : null),
                        valid_elements: "*[*]",
                        setup: function(editor) {
                            var ediWin, ediDoc, ediDocBody,
                                tmpCtnr = $('<div></div>'),
                                tmpDoc,
                                getTidyContents = function() {
                                    var contents = ediDocBody.innerHTML;
                                    if (contents) {
    ...
    (function() {
        ...
        var loadItemFunc = function(itemBody) {
            ...
            var dragContainerClass = "*[connect\\:class~='gapChoiceContainer']";
            ...
            interactions.each(function(index, interaction) {
                ...
                var dragItems = interaction.find("*[connect\\:class~='" + dragTargetClass + "']");
                var dropItems = interaction.find("*[connect\\:class~='" + dropTargetClass + "']");
                var svgStimulus = dropItems.parents("[connect\\:author-class~='svg-interactions-container']").length > 0;
                ...
                gapChoiceItems.each(function(index, gapChoiceItem) {
                    gapChoiceItem = $(gapChoiceItem);
                    sizeSpan.text(gapChoiceItem.text());
                ...
                // sets max field with of drop areas
                if (svgStimulus) {
                    width = 0;
                } else if (width > 75) {
                    width = 75;
                ...
                (function() {
                        ...
                        if (!dragObject) {
                            ...
                            dragObject.undoDrag = function(switchElm) {
                                ...
                                if (insertNode.parent().length > 0) {
                                    ...
                                    if (dragParent.css("min-width") === "0px") {
                    ...
                    dropItems.each(function(index, dropItem) {
                        ...
                        dropItem.css("padding", "0 " + width + "px");
                            ...
                            if (draggedItem) {
                                if (isDragItem.test(draggedItem.attr("connect:class"))) {
                                    ...
                                    if (dropItem.css("min-width") !== "0px") {
                                        ...
                                        dropItem.css("left", dropLeft + dropOffset - dragOffset + "px");
                                    } else {
                    ...
                    function $makeDraggable(elm, options) {
                        ...
                        var settings = {
                            ...
                            //appendTo: '[connect\\:class~="gapMatchInteraction"]:visible',
                            appendTo: '[connect\\:class~="gapMatchInteraction"][connect\\:responseIdentifier="' + responseIdentifier + '"]',
                            zIndex: 100
                        };
                        if (options) {
        ...
        var testStartFunc = function(item) {
            var savedRspVal,
    ...
    (function() {
        ...
        var loadItemFunc = function(itemBody) {
            ...
            if (twoColChoiceContainer.length > 0) {
                ...
                //qti.subscribeToEvent("windowResize", ensureSameChoiceHeight, "Make choices the same height", itemIdentifier+"2colChoices_heightSet_windowResize");
                //qti.subscribeToEvent("accessChange", ensureSameChoiceHeight, "Make choices the same height", itemIdentifier+"2colChoices_heightSet_accessChange");
                ...
                interactionStages.each(function(index, interactionStage) {
                    ...
                    (function() {
                        ...
                        function layoutDragItems() {
                            ...
                            dragItems.each(function(i) {
                                var dragItem = $(this),
                                    qDfrd = $.Deferred(),
                                    lDfrd = $.Deferred(),
                                    imageElm = dragItem.find('img,object'),
                                ...
                                imageElm.on('load', function() {
                                    lDfrd.resolve();
                                });
                                imgLoads.push(lDfrd.promise());
                                imgQ.push((i === 0 ? $.Deferred().resolve().promise() : imgQ[i - 1]).then(function() {
                                    imgLoads[i].done(function() {
                                        var imageHeight = imageElm.height() || imageElm.outerHeight();
                                        setImageHeight(imageHeight);
                                    });
                                    return qDfrd.promise();
                                }));
                            });
                        }
                        ...
                        function saveMultipleResponse(responseObject) {
                            var responseArray = [];
                            responseObject.floatingItems.forEach(function(floatingItem) {
                                responseArray.push(floatingItem.position);
                            });
                        ...
                        //Get the index of a floating item associated with
                        //a given drag object
                        ...
                        function getDragObject(item) {
                            ...
                            if (!dragObject) {
                                ...
                                insertNode.attrNS('connect:responseIdentifier', item.attr('connect:responseIdentifier'));
                                insertNode.attr('draggable', item.attr('draggable'));

                                //Response handling
                                var responseIdentifier = item.attr('connect:responseIdentifier') || item.parent().attr('connect:responseIdentifier');
                                var responseVariable = qti.getVariable(responseIdentifier, itemIdentifier);
                                var cardinality = responseVariable.getCardinality();
                                var baseType = responseVariable.getBaseType();

                                var valueParsers = $.createHash('point', qti.parsePoint);
                        ...
                        dropItems.each(function(index, dropItem) {
                            ...
                            var drop = function(event, dropPosition) {
                                ...
                                var draggedItem = localDataObject.insertNode;
                                var draggedPosition = dragData.mouseOffset;
                                var insertWidth = Math.round(parseInt($('object', draggedItem).attr('width'), 10));

                                //If image instead of object, recalculate insertWidth
                                if (!insertWidth) {
                                    insertWidth = Math.round(parseInt($('img', draggedItem).attr('width'), 10));
                                }
                                ...
                                if (draggedItem) {
                                    if (isDragItem.test(draggedItem.attr('connect:class'))) {
                                        ...
                                        if (!dragData.floating && !dragData.toDelete) {
                                            ...
                                            responseValue = localDataObject.responseData.parseValue(draggedPosition.left +
                                                " " + draggedPosition.top);
                        ...
                        dragItems.each(function(index, dragItem) {
                            ...
                            function keyDown(event) {
                                ...
                                if (arrowCursor) {
                                    ...
                                    //Save response value
                                    responseValue = dragObject.responseData.parseValue(currentX +
                                        " " + currentY);
    ...
    (function() {
        ...
        var loadItemFunc = function(itemObj) {
            var itemBody = itemObj.body;
            var calculators = $('[connect\\:class~="calculator"]');
                ...
                engine = (function() {
                    ...
                    operators = {
                        "+": {
                            ...
                            exec: function() {
                                // carry out addition
                                accumulator.value = performCalculation("+");
                            }
                        },
                        "−": {
                            name: "−",
                    ...
                    performCalculation = function(operation) {
                        ...
                        return roundResult(result, MAX_DIGITS + 1);
    ...
    (function() {
        ...
        function createLine(lineContainer, sourceX, sourceY, targetX, targetY) {
                ...
                matchTargets.each(function(index, matchTarget) {
                    ...
                    matchTarget.click(function(event) {
                        var targetCoords, svgCoords, existingGroup, sourceMatchedWith, sourceDiv,
                            innerDiv = $(matchTarget.find("div")[0]),
                ...
                matchSources.each(function(index, matchSource) {
                    ...
                    matchSource.click(function(event) {
                        var sourceCoords, svgCoords, existingGroup,
                            innerDiv = $(matchSource.find("div")[0]),
                            matchMax = Number(innerDiv.attr("connect:matchMax")),
                            matchedWith = innerDiv.attr("connect:matchedWith"),
        ...
        var screenDisplay = function(itemBody) {
            ...
            if (matchInteractions.length > 0) {
                /*$('[role="main"] [connect\\:class~="activeItem"]').each(function() {
                	drawRecoveredResponses($(this));
                });*/
    ...
    (function() {
        function loadItemFunc(itemBody) {
            //var itemBody = itemObj.body;
            var pvElements = itemBody.find('*[connect\\:class~="printedVariable"]');
            pvElements.each(function(index, pvElement) {
                pvElement = $(pvElement);
                var variableIdentifier = pvElement.attr('connect:identifier');
                var variable = qti.getVariable(variableIdentifier);
